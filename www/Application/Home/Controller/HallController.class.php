...
class HallController extends PublicController{
    ...
    {
		$model = D('HallView');
        $user = $model -> field('username,team,sum(points) as sum_points,pid,avatar,create_at') -> where('points > 0 and email <> \'0\'') -> order("sum_points desc") -> group('user_id') -> select();
    ...
    public function sortdata()
    {
        ...
        $hallResult = $model -> where($map) -> where('points > 0 and email <> \'0\'') -> field('username,team,sum(points) as sum_points,pid,avatar') -> order("sum_points desc") -> group('user_id') -> select();
		if (!empty($hallResult)){
